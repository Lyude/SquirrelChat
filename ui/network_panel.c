/* Code for the nice network panel on the side of the window
 */
...
void create_network_panel() {
    ...
    network_tree_store = gtk_tree_store_new(2, G_TYPE_STRING, G_TYPE_POINTER);
    network_tree = gtk_tree_view_new_with_model(GTK_TREE_MODEL(network_tree_store));
    network_tree_title_column = gtk_tree_view_column_new();
    network_tree_data_column = gtk_tree_view_column_new();
    gtk_tree_view_append_column(GTK_TREE_VIEW(network_tree), network_tree_title_column);
    gtk_tree_view_append_column(GTK_TREE_VIEW(network_tree), network_tree_data_column);

    network_tree_renderer = gtk_cell_renderer_text_new();
    gtk_tree_view_column_pack_start(network_tree_title_column, network_tree_renderer,
                                    TRUE);
    gtk_tree_view_column_add_attribute(network_tree_title_column, network_tree_renderer,
                                       "text", 0);
    gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(network_tree), FALSE);
    gtk_tree_selection_set_mode(gtk_tree_view_get_selection(
                                GTK_TREE_VIEW(network_tree)),
                                GTK_SELECTION_SINGLE);
}

// Adds an empty network buffer to the network panel
void add_network(struct network_buffer * buffer) {
    gtk_tree_store_append(GTK_TREE_STORE(network_tree_store),
                          &network_tree_toplevel, NULL);
    gtk_tree_store_set(GTK_TREE_STORE(network_tree_store), &network_tree_toplevel,
...
void cursor_changed_handler(GtkTreeSelection *treeselection) {
    ...
    gtk_text_view_set_buffer(GTK_TEXT_VIEW(main_text_view), network->buffer);
}

/* Connects the signals for the network panel
...
void connect_network_panel_signals() { 
    g_signal_connect(gtk_tree_view_get_selection(GTK_TREE_VIEW(network_tree)), "changed",
                     G_CALLBACK(cursor_changed_handler), main_text_view);
}
